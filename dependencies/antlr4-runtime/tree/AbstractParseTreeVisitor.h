...
namespace tree {
  ...
  public:
    ...
    virtual std::any visit(ParseTree *tree) override {
      ...
      auto t = dynamic_cast<ParserRuleContext *>(tree);
      if (parent != nullptr && t != nullptr) {
        t->while_entry = parent->while_entry;
        t->while_exit = parent->while_exit;
        t->while_body = parent->while_body;
      }
