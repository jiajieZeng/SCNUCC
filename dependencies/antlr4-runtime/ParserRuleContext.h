...
namespace antlr4 {
  ...
  public:
    std::shared_ptr<SCNUCC::Block> while_entry;
    std::shared_ptr<SCNUCC::Block> while_body;
    std::shared_ptr<SCNUCC::Block> while_exit;
    // std::vector<std::shared_ptr<BRIR>> false_list;  // 真出口链
    // std::vector<std::shared_ptr<BRIR>> true_list;  // 假出口链

    bool while_is_finished = false;  // 用于标记while循环是否被break、continue
